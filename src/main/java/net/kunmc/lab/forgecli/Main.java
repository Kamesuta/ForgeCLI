...
public class Main {
    public static void main(String[] args) throws Throwable {
        ...
        if (argsList.size() < 6) {
            System.out.println("Invalid arguments! Use: java -jar <ForgeCLI.jar> --installer <forge-installer.jar> --target <install-location> --forgeVersion <forget-version>");
            return;
        }
        ...
        String forgeVersion = argsList.get(argsList.indexOf("--forgeVersion") + 1);
        ...
        try (URLClassLoader ucl = URLClassLoader.newInstance(new URL[]{
            ...
            if (!(boolean) installer.getMethod("install", File.class, File.class, String.class).invoke(null, targetDir.toFile(), installerJar.toFile(), forgeVersion)) {
                return;
            }
        }
